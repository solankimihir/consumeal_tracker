/* AUTOMATICALLY GENERATED CODE DO NOT MODIFY */
/*   To generate run: "serverpod generate"    */

// ignore_for_file: implementation_imports
// ignore_for_file: library_private_types_in_public_api
// ignore_for_file: non_constant_identifier_names
// ignore_for_file: public_member_api_docs
// ignore_for_file: type_literal_in_constant_pattern
// ignore_for_file: use_super_parameters

library protocol; // ignore_for_file: no_leading_underscores_for_library_prefixes

import 'package:serverpod/serverpod.dart' as _i1;
import 'package:serverpod/protocol.dart' as _i2;
import 'example.dart' as _i3;
import 'items_in_a_meal.dart' as _i4;
import 'master_items.dart' as _i5;
import 'meals.dart' as _i6;
import 'package:consumeal_tracker_server/src/generated/master_items.dart'
    as _i7;
import 'package:consumeal_tracker_server/src/generated/meals.dart' as _i8;
import 'package:consumeal_tracker_server/src/generated/items_in_a_meal.dart'
    as _i9;
export 'example.dart';
export 'items_in_a_meal.dart';
export 'master_items.dart';
export 'meals.dart';

class Protocol extends _i1.SerializationManagerServer {
  Protocol._();

  factory Protocol() => _instance;

  static final Protocol _instance = Protocol._();

  static final List<_i2.TableDefinition> targetTableDefinitions = [
    _i2.TableDefinition(
      name: 'items_in_a_meal',
      dartName: 'ItemsInAMeal',
      schema: 'public',
      module: 'consumeal_tracker',
      columns: [
        _i2.ColumnDefinition(
          name: 'id',
          columnType: _i2.ColumnType.bigint,
          isNullable: false,
          dartType: 'int?',
          columnDefault: 'nextval(\'items_in_a_meal_id_seq\'::regclass)',
        ),
        _i2.ColumnDefinition(
          name: 'mealIDId',
          columnType: _i2.ColumnType.bigint,
          isNullable: false,
          dartType: 'int',
        ),
        _i2.ColumnDefinition(
          name: 'itemIDId',
          columnType: _i2.ColumnType.bigint,
          isNullable: false,
          dartType: 'int',
        ),
        _i2.ColumnDefinition(
          name: 'quantity',
          columnType: _i2.ColumnType.doublePrecision,
          isNullable: false,
          dartType: 'double',
        ),
        _i2.ColumnDefinition(
          name: 'remarks',
          columnType: _i2.ColumnType.text,
          isNullable: false,
          dartType: 'String',
        ),
        _i2.ColumnDefinition(
          name: 'calories',
          columnType: _i2.ColumnType.bigint,
          isNullable: false,
          dartType: 'int',
        ),
      ],
      foreignKeys: [
        _i2.ForeignKeyDefinition(
          constraintName: 'items_in_a_meal_fk_0',
          columns: ['mealIDId'],
          referenceTable: 'meals',
          referenceTableSchema: 'public',
          referenceColumns: ['id'],
          onUpdate: _i2.ForeignKeyAction.noAction,
          onDelete: _i2.ForeignKeyAction.noAction,
          matchType: null,
        ),
        _i2.ForeignKeyDefinition(
          constraintName: 'items_in_a_meal_fk_1',
          columns: ['itemIDId'],
          referenceTable: 'master_items',
          referenceTableSchema: 'public',
          referenceColumns: ['id'],
          onUpdate: _i2.ForeignKeyAction.noAction,
          onDelete: _i2.ForeignKeyAction.noAction,
          matchType: null,
        ),
      ],
      indexes: [
        _i2.IndexDefinition(
          indexName: 'items_in_a_meal_pkey',
          tableSpace: null,
          elements: [
            _i2.IndexElementDefinition(
              type: _i2.IndexElementDefinitionType.column,
              definition: 'id',
            )
          ],
          type: 'btree',
          isUnique: true,
          isPrimary: true,
        )
      ],
      managed: true,
    ),
    _i2.TableDefinition(
      name: 'master_items',
      dartName: 'MasterItems',
      schema: 'public',
      module: 'consumeal_tracker',
      columns: [
        _i2.ColumnDefinition(
          name: 'id',
          columnType: _i2.ColumnType.bigint,
          isNullable: false,
          dartType: 'int?',
          columnDefault: 'nextval(\'master_items_id_seq\'::regclass)',
        ),
        _i2.ColumnDefinition(
          name: 'name',
          columnType: _i2.ColumnType.text,
          isNullable: false,
          dartType: 'String',
        ),
        _i2.ColumnDefinition(
          name: 'unit',
          columnType: _i2.ColumnType.text,
          isNullable: false,
          dartType: 'String',
        ),
        _i2.ColumnDefinition(
          name: 'calories',
          columnType: _i2.ColumnType.doublePrecision,
          isNullable: false,
          dartType: 'double',
        ),
      ],
      foreignKeys: [],
      indexes: [
        _i2.IndexDefinition(
          indexName: 'master_items_pkey',
          tableSpace: null,
          elements: [
            _i2.IndexElementDefinition(
              type: _i2.IndexElementDefinitionType.column,
              definition: 'id',
            )
          ],
          type: 'btree',
          isUnique: true,
          isPrimary: true,
        )
      ],
      managed: true,
    ),
    _i2.TableDefinition(
      name: 'meals',
      dartName: 'Meals',
      schema: 'public',
      module: 'consumeal_tracker',
      columns: [
        _i2.ColumnDefinition(
          name: 'id',
          columnType: _i2.ColumnType.bigint,
          isNullable: false,
          dartType: 'int?',
          columnDefault: 'nextval(\'meals_id_seq\'::regclass)',
        ),
        _i2.ColumnDefinition(
          name: 'datetimestamp',
          columnType: _i2.ColumnType.timestampWithoutTimeZone,
          isNullable: false,
          dartType: 'DateTime',
        ),
        _i2.ColumnDefinition(
          name: 'name',
          columnType: _i2.ColumnType.text,
          isNullable: false,
          dartType: 'String',
        ),
        _i2.ColumnDefinition(
          name: 'remarks',
          columnType: _i2.ColumnType.text,
          isNullable: false,
          dartType: 'String',
        ),
      ],
      foreignKeys: [],
      indexes: [
        _i2.IndexDefinition(
          indexName: 'meals_pkey',
          tableSpace: null,
          elements: [
            _i2.IndexElementDefinition(
              type: _i2.IndexElementDefinitionType.column,
              definition: 'id',
            )
          ],
          type: 'btree',
          isUnique: true,
          isPrimary: true,
        )
      ],
      managed: true,
    ),
    ..._i2.Protocol.targetTableDefinitions,
  ];

  @override
  T deserialize<T>(
    dynamic data, [
    Type? t,
  ]) {
    t ??= T;
    if (t == _i3.Example) {
      return _i3.Example.fromJson(data) as T;
    }
    if (t == _i4.ItemsInAMeal) {
      return _i4.ItemsInAMeal.fromJson(data) as T;
    }
    if (t == _i5.MasterItems) {
      return _i5.MasterItems.fromJson(data) as T;
    }
    if (t == _i6.Meals) {
      return _i6.Meals.fromJson(data) as T;
    }
    if (t == _i1.getType<_i3.Example?>()) {
      return (data != null ? _i3.Example.fromJson(data) : null) as T;
    }
    if (t == _i1.getType<_i4.ItemsInAMeal?>()) {
      return (data != null ? _i4.ItemsInAMeal.fromJson(data) : null) as T;
    }
    if (t == _i1.getType<_i5.MasterItems?>()) {
      return (data != null ? _i5.MasterItems.fromJson(data) : null) as T;
    }
    if (t == _i1.getType<_i6.Meals?>()) {
      return (data != null ? _i6.Meals.fromJson(data) : null) as T;
    }
    if (t == List<_i7.MasterItems>) {
      return (data as List).map((e) => deserialize<_i7.MasterItems>(e)).toList()
          as dynamic;
    }
    if (t == List<_i8.Meals>) {
      return (data as List).map((e) => deserialize<_i8.Meals>(e)).toList()
          as dynamic;
    }
    if (t == List<_i9.ItemsInAMeal>) {
      return (data as List)
          .map((e) => deserialize<_i9.ItemsInAMeal>(e))
          .toList() as dynamic;
    }
    try {
      return _i2.Protocol().deserialize<T>(data, t);
    } on _i1.DeserializationTypeNotFoundException catch (_) {}
    return super.deserialize<T>(data, t);
  }

  @override
  String? getClassNameForObject(Object? data) {
    String? className = super.getClassNameForObject(data);
    if (className != null) return className;
    if (data is _i3.Example) {
      return 'Example';
    }
    if (data is _i4.ItemsInAMeal) {
      return 'ItemsInAMeal';
    }
    if (data is _i5.MasterItems) {
      return 'MasterItems';
    }
    if (data is _i6.Meals) {
      return 'Meals';
    }
    className = _i2.Protocol().getClassNameForObject(data);
    if (className != null) {
      return 'serverpod.$className';
    }
    return null;
  }

  @override
  dynamic deserializeByClassName(Map<String, dynamic> data) {
    if (data['className'] == 'Example') {
      return deserialize<_i3.Example>(data['data']);
    }
    if (data['className'] == 'ItemsInAMeal') {
      return deserialize<_i4.ItemsInAMeal>(data['data']);
    }
    if (data['className'] == 'MasterItems') {
      return deserialize<_i5.MasterItems>(data['data']);
    }
    if (data['className'] == 'Meals') {
      return deserialize<_i6.Meals>(data['data']);
    }
    if (data['className'].startsWith('serverpod.')) {
      data['className'] = data['className'].substring(10);
      return _i2.Protocol().deserializeByClassName(data);
    }
    return super.deserializeByClassName(data);
  }

  @override
  _i1.Table? getTableForType(Type t) {
    {
      var table = _i2.Protocol().getTableForType(t);
      if (table != null) {
        return table;
      }
    }
    switch (t) {
      case _i4.ItemsInAMeal:
        return _i4.ItemsInAMeal.t;
      case _i5.MasterItems:
        return _i5.MasterItems.t;
      case _i6.Meals:
        return _i6.Meals.t;
    }
    return null;
  }

  @override
  List<_i2.TableDefinition> getTargetTableDefinitions() =>
      targetTableDefinitions;

  @override
  String getModuleName() => 'consumeal_tracker';
}
